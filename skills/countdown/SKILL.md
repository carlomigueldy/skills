...
printf '\nENV_MATCH='; env | awk -F= '{print $1}' | grep -E '^(CODEX_|OPENCODE)' | tr '\n' ' '
printf '\nCLAUDECODE=%s\n' "${CLAUDECODE:-}"
printf 'PWD=%s HOME=%s GIT=%s\n' "$PWD" "$HOME" "$(git rev-parse --is-inside-work-tree 2>/dev/null || echo no)"
```

(The env scan prints variable NAMES only, never values — that's all detection needs.)

Check in this order — first match wins:

1. **Claude Cowork (cloud VM)** if ANY: `CLAUDE_CODE_REMOTE` / `CLAUDE_CODE_REMOTE_SESSION_ID` is set; `/mnt/user-data` or `/mnt/skills` exists; `$PWD` / `$HOME` sits under `/mnt/` or `/sessions/`. There is no local terminal to open a second session here.
2. **OpenAI Codex CLI** if any `CODEX_*` variable is present. VERIFIED live inside a real Codex session — `CODEX_SANDBOX`, `CODEX_SANDBOX_NETWORK_DISABLED`, `CODEX_THREAD_ID`, and others in the same family were observed set. Treat any `CODEX_*` variable as a reliable primary signal, not just a hint.
3. **OpenCode** if any `OPENCODE*` variable is present — the opencode binary confirms an `OPENCODE_*` family (`OPENCODE_CLIENT`, `OPENCODE_PERMISSION`, `OPENCODE_CONFIG`, `OPENCODE_WORKSPACE_ID`, and others), but a live child-shell export wasn't verified end-to-end. Treat it as a strong hint — confirm via the fallback question if anything else conflicts.
4. **Claude Code CLI** if NONE of the above AND `$HOME` is a real user home (`/home/*`, `/Users/*`); a git work tree corroborates, and a non-empty `CLAUDECODE` is a supporting hint.
5. **Ambiguous** → one AskUserQuestion ("Where will the build run?") offering all four platforms, each option carrying a populated per-option `description` per AskUserQuestion rule 4. No platform publishes an official session marker for this purpose, so this heuristic plus the fallback question is the whole detection story.

Carry the detected platform forward — it flips the STEP 0.5 recommendation and tailors the hand-off delivery mechanics. For everything downstream, OpenAI Codex CLI and OpenCode behave exactly like Claude Code CLI: local filesystem, local terminal, same recommendation logic, copyable-absolute-paths delivery.
...
- **"Just give me the build hand-off prompt"** — assemble the `ultracode:` prompt, save BUILD-HANDOFF.md, STOP; you start the build session yourself. Trade-off: clean context separation, but the build runs elsewhere. *Recommended on any local CLI — Claude Code, OpenAI Codex CLI, or OpenCode.*
...
3. Present the hand-off prompt in a copyable block AND save it as BUILD-HANDOFF.md alongside the PRD and prototype files — presented per the DELIVERABLE PRESENTATION rule. Then give platform-tailored delivery instructions (from STEP 0):
   ...
   - **OpenAI Codex CLI / OpenCode** → same mechanics as Claude Code CLI: enter the target directory, start the CLI (`codex` / `opencode`), paste the prompt, and point it at the same files by copyable absolute path.
