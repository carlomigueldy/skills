...
- `../saas-launch-blueprint/references/interaction-rules.md` — the Delegation Model (fan market/competitor research out to parallel subagents; keep scoring judgments yourself), AskUserQuestion rules 1–8 (write-up-BEFORE-widget, the option-cap rule 7, per-decision write-ups when chaining segment → scope → idea, the Presentation Environment + rule-6 Comparison Format policy: widget in Cowork vs. rich markdown scorecard table in Claude Code / the TUI, and rule 8's plain-text numbered-list fallback for clients where AskUserQuestion itself is unavailable), Known Failure Mode #1 (a widget with no context — why the standalone write-up must land visibly first), and the Approval Gates hard rule (the three standing options every gate carries).
