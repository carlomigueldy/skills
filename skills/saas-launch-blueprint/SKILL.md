...
description: End-to-end guided workflow for ideating, speccing, prototyping, and handing off a bootstrapped SaaS MVP for a solo founder (Philippine or worldwide market) — target-market selection, scored product ideation, interview-driven PRD via doc-coauthoring, high-fidelity HTML prototype, and a Phase 3.5 build fork that either runs the build in-session to delivery (GATE 4) or produces an ultracode build-handoff prompt for a separate build session (Claude Code, OpenAI Codex CLI, or OpenCode). Use this skill whenever the user wants to start a new SaaS or product idea, brainstorm a cash-cow app, run "Phase 1", build a PRD for a bootstrapped product, mentions the Launch Blueprint or master prompt, or asks to ideate/validate/prototype a subscription product with manual payment collection (GCash, Maya, PayPal, crypto) — even if they don't name this skill explicitly.
---
...
The workflow is sequential and gated: never skip ahead, never start a phase without the previous gate's explicit approval. Phases 1–3 never build; the build starts only at the countdown fork, in-session on "Build it out now," otherwise in a separate session.
...
At the start of every run, create a task list mirroring the phases and gates (Phase 1 → Gate 1 → Phase 2 → Gate 2 → Phase 3 → Gate 3 → Phase 3.5 (Handoff OR in-session Build) → STOP) and keep it updated as you go — it is the user's progress map. Then begin Phase 1 by invoking the Phase 1 skill.
...
- On Gate 3 **"Approve — prepare build handoff"** (or after a "Validate first" reconvene that then approves): invoke `saas-launch:countdown` (Phase 3.5). countdown ALWAYS asks build-now vs hand-off: on **"Just give me the build hand-off prompt"** it assembles the hand-off prompt and STOPs; on **"Build it out now"** it runs Phase 4 in-session through **GATE 4** ("Accept delivery" / "Request changes") and STOPs there instead. Either way, control never returns here — terminal for this orchestrator.
...
- Reiterate the session boundary: Phases 1–3 never build; the build starts only at the countdown fork — in-session on "Build it out now," otherwise in a separate session using the Phase 3.5 hand-off prompt.
...
| Rule | Canonical home |
|---|---|
| Delegation Model; Known Failure Modes; Presentation Environment (client detection); AskUserQuestion rules 1–8 (incl. rule 6 environment-aware Comparison Format); Approval Gates hard rule; Deliverable Presentation hard rule | `references/interaction-rules.md` |
