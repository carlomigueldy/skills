...
Run `clean-state-checklist.md` in full before ending any session — no
`in_progress` feature, `./init.sh verify` and `./init.sh test` both green,
`git status` clean (or every remaining change intentional and committed),
memory files current, no debug debris. For Claude Code, the **Stop gate** in
`.claude/settings.json` enforces the single hardest requirement
mechanically: it blocks session end (exit 2) while any feature is
`in_progress`. The rest of the checklist is on the agent's discipline, same
as every other tier.

## Self-evaluation

Before `qa-verifier` flips a feature to `passing`, score it against
`evaluator-rubric.md`: 6 dimensions, 0–2 each. **Pass requires total >= 10
AND no dimension scored 0.** Anything short of that is `blocked` with notes
naming the specific gap — never `passing` on a partial score.

## Observability

Every agent action of consequence appends a JSONL event to
`logs/agent-events.jsonl` — append-only, never delete or rewrite prior
lines. Each event: `ts` (ISO 8601), `agent` (which crew role or model
fired it), `event` (what happened), `feature_id` (the `feature_list.json`
id it relates to, or null), `detail` (short free-text). This is the audit
trail an unattended run leaves behind — treat a missing or edited log line
as a bug in the run, not a cosmetic gap.

## Autonomous loop

When running unattended (no human in the room to redirect), follow
`autonomous-loop.md` verbatim: read memory → pick the highest-priority
`not_started` feature → mark `in_progress` → route via `workflows/` → crew
implements → `qa-verifier` verifies → score against the rubric → `passing`
+ evidence (or `blocked` + notes) → update memory → repeat. The Stop gate is
the loop's exit contract — it will not let the session end mid-feature.
